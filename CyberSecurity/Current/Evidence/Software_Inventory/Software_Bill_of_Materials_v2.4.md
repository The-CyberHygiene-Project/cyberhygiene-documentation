...
|----------|-----|----------|--------------|----------|------|
| **dc1.cyberinabox.net** | 192.168.1.10 | Rocky Linux 9.7 | x86_64 | ~1,776 | Domain Controller, FIPS enabled |
...
**Total Software Packages:** ~5,626 across all systems
...
| 2.4 | 2026-02-17 | D. Shannon | Added SIEM/monitoring stack: OpenSearch 2.19.4 (search/analytics), Wazuh Dashboard 4.14.3 (SIEM UI), Filebeat 7.10.2 (alert forwarding). Added Prometheus node_exporter 1.10.2 for dc1 host metrics. Removed OpenClaw AI Security Infrastructure (decommissioned). Updated wazuh-manager 4.14.2→4.14.3, grafana 12.3.1→12.3.3, mongodb-org 7.0.28→7.0.30. Kernel updated to 5.14.0-611.30.1. |
...
**Architecture:** x86_64
**FIPS Mode:** Enabled
**Total Packages:** 1,776 RPM + 1 Flatpak
**Role:** FreeIPA Domain Controller, Primary DNS, Samba File Server, Wazuh SIEM (Manager + Dashboard + OpenSearch), Graylog, Prometheus Monitoring, Web Server, NextCloud, Email Server
...
#### Security & Monitoring
- **wazuh-manager** 4.14.3-1 - Wazuh SIEM Manager
- **wazuh-dashboard** 4.14.3-1 - Wazuh SIEM Web Dashboard (OpenSearch Dashboards fork)
  - Port: 5601 (HTTPS)
  - Backend: OpenSearch 2.19.4
  - Authentication: OpenSearch Security internal users database
  - License: GPL v2
- **graylog-server** 6.1.16-1 - Centralized Log Management
- **grafana** 12.3.3-1 - Metrics Visualization
...
#### SIEM Data Stack (NEW - February 17, 2026)
- **opensearch** 2.19.4-1 - Search and analytics engine (Wazuh/SIEM backend)
  - Port: 9200 (HTTPS, localhost only)
  - Security: OpenSearch Security plugin enabled, TLS transport + HTTP
  - Client certificate auth + basic auth
  - FIPS-compliant TLS (RSA 2048 + SHA256)
  - Discovery: single-node
  - License: Apache 2.0
- **filebeat** 7.10.2-2 - Lightweight log shipper (Wazuh alert forwarding)
  - Module: wazuh (alerts)
  - Input: /var/ossec/logs/alerts/alerts.json
  - Output: OpenSearch (HTTPS with client certificate)
  - Index pattern: wazuh-alerts-4.x-*
  - License: Elastic License / Apache 2.0

#### Monitoring & Metrics
- **prometheus** 2.48.0 - Time-series metrics collection and alerting
  - Port: 9091 (HTTP, localhost only)
  - Storage: /var/lib/prometheus/
  - Scrape targets: 7 (dc1, 4 workstations, suricata, self)
  - License: Apache 2.0
- **node_exporter** 1.10.2 - Prometheus host metrics exporter (NEW - February 17, 2026)
  - Port: 9100 (HTTPS)
  - TLS: SSL.com wildcard certificate (*.cyberinabox.net)
...
#### Database Services
- **mongodb-org** 7.0.30-1.el9 - Document database (Graylog backend)
...
- **Wazuh Packages (manager, dashboard, filebeat module):** Wazuh official repository (packages.wazuh.com)
- **OpenSearch:** Wazuh repository (bundled compatible version)
- **Verification:** GPG signature verification via Wazuh repo key
- **Trust:** Open source (Apache 2.0 / GPL v2)

**Prometheus Ecosystem:**
- **Source:** GitHub official releases (prometheus/prometheus, prometheus/node_exporter)
- **Verification:** SHA-256 checksums verified
- **Trust:** CNCF graduated project, community-vetted
